...
from agent.verSum.SA_ClientAgent import SA_ClientAgent
from agent.verSum.SA_CollectionServer import SA_CollectionServer
from agent.verSum.SA_DecryptionServer import SA_DecryptionServer
from agent.verSum.SA_RegistrationService import SA_RegistrationService
...
if dataset.upper() == "EMNIST-DIGITS":
    ...
    (X_train_raw, y_train_raw), (X_test_raw, y_test_raw) = cifar100.load_data(label_mode='fine')
    X_input = np.concatenate([X_train_raw, X_test_raw])
    y_input = np.concatenate([y_train_raw, y_test_raw]).ravel()
    X_input = X_input.reshape(X_input.shape[0], -1).astype(np.float32) / 255.0
    scaler = StandardScaler()
    scaler.fit(X_input)
...
agent_types.extend(["SA_DecryptionServer"])
agent_count += 1
agents.extend([ SA_DecryptionServer(
                id = b, name = "SA_DecryptionServer",
                type = "SA_DecryptionServer",
...
agent_count += 1
agents.extend([ SA_RegistrationService(
                id = b+1, name = "SA_DecryptionServer",
                type = "SA_DecryptionServer",
                random_state = np.random.RandomState(seed=np.random.randint(low=0,high=2**32, dtype='uint64')),
                iterations = num_iterations,
                ) ])
